...
void LotSizingSolver::solveEquationSystem_holding_backward(std::shared_ptr<LinearPiece> C,
                                          std::shared_ptr<LinearPiece> fromC,
                                          std::shared_ptr<LinearPiece> fromF,
                                          double I, double demand,
                                          double &fromI, double &quantity,std::shared_ptr<LinearPiece> Cpre,double hodling){
...
bool LotSizingSolver::backtracking_stockout_backward() {
  // initialization
  for (int i = 0; i < horizon; i++){
    quantities[i] = 0.0;
    breakpoints[i] = nullptr;
  ...
  // std::cout << "stock at the end of day " << day << " " << I[day] << std::endl;
  ///////////////////////////////////////////
  while (tmp != nullptr){
    ...
    else if (eq(tmp->replenishment_loss,-1) && tmp->fromF ) //f3
    {
      ...
      solveEquationSystem_holding_backward(tmp, fromC, fromF, I[day],
                            params->cli[client].dailyDemand[day], I[day + 1],
    ...
    // std::cout << "quantity to deliver at day " << day+1 << " " << quantities[day] << std::endl;
    // std::cout << "stock at the end of day " << day+1 << " " << I[day+1] << std::endl;
    day = day + 1;
...
bool LotSizingSolver::backtracking_stockout(){
  ...
  // std::cout << "stock at the end of day " << day+1 << " " << I[day] << std::endl;
  ...
  if(eq(tmp->replenishment_loss,-1)){
    ...
    if (breakpoints[day] && fabs(quantities[day])<1) {
      throw string("test");
    }
  }
...
void  LotSizingSolver::Firstday(vector<std::shared_ptr<PLFunction>> &C){//for the first day
  ...
  if (I < 0)
  {
    ...
    Cn1 = Cn1->getInBound(1, std::min<double>(params->cli[client].maxInventory, params->cli[client].dailyDemand[0] - 1) - params->cli[client].startingInventory, false);
  ...
  Cnew= Cnew->getInBound(std::max<double>(1,params->cli[client].dailyDemand[0]-params->cli[client].startingInventory), 
...
bool LotSizingSolver::solve_stockout_backward()
{
  ...
  {
    //picewise linear functions
    std::shared_ptr<PLFunction> partialf1;
    std::shared_ptr<PLFunction> f1(std::make_shared<PLFunction>(params));
    ...
    std::shared_ptr<PLFunction>  fromC2(std::make_shared<PLFunction>(params));
    minInventory = params->cli[client].minInventory;
    ...
    partialf1 = copyPLFunction(C[t]); //partialF1(x) = Ct(x)
    partialf1->addHoldingf(params->cli[client].inventoryCost); //partialF1(x) = Ct(x) + hi*x

    for (int i = 0; i < partialf1->nbPieces; i++)
    {
      partialf1->pieces[i]->fromC_pre = C[t]->pieces[i]->clone();
      partialf1->pieces[i]->replenishment_loss = -1; 
      partialf1->pieces[i]->fromF = nullptr;
      partialf1->pieces[i]->fromC = partialf1->pieces[i]->clone();
      partialf1->pieces[i]->fromInst = nullptr;
    }
    ...
    shared_ptr<LinearPiece> tmp(make_shared<LinearPiece>(0, params->cli[client].dailyDemand[t-1] * params->cli[client].stockoutCost, params->cli[client].dailyDemand[t-1], 0));
    f2->append(tmp); //f2(x) = stockoutCost * (daily - x), x < d_i^t
    for (int i = 0; i < f2->nbPieces; i++){
    ...
    if (neq(C[t]->pieces[0]->p1->x,0)) {
      std::cout << C[t]->pieces[0]->p1->x << std::endl;
      throw string("error");
    }
    f2->moveUp(C[t]->pieces[0]->p1->y); //f2(x) = C_t(0) + stockoutCost * (daily - x), x < d_i^t
    
    f2 = f2->getInBound(0, params->cli[client].dailyDemand[t-1], false); //x < d_i^t
    ...
    fromF2 = fromF2->getInBound(-params->cli[client].maxInventory,-1,false); //q < maxInventory
    ...
    shared_ptr<LinearPiece> tmptmp(make_shared<LinearPiece>(0, params->cli[client].stockoutCost * params->cli[client].dailyDemand[t-1], params->cli[client].dailyDemand[t-1], 0));
    fromC2->append(tmptmp); //fromC2(x) = stockoutCost * (d_i^t-x)
    fromC2 = fromC2->getInBound(0,params->cli[client].dailyDemand[t-1], false); // x < d_i^t
    fromC2->moveUp(C[t]->pieces[0]->p1->y); //add C_t(0)
    
    fromF1 = copyPLFunction(F1[t-1]); //fromF1(q) = F_1^t(q)
    fromF1 = fromF1->getInBound(-(params->cli[client].dailyDemand[t-1]),-1,false); //q < d_i^t
  ...
  bool ok = backtracking_stockout_backward();
